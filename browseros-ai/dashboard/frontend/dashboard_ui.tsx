...
const themeTokens: Record<
  ThemeMode,
  { page: string; panel: string; text: string; muted: string; border: string }
> = {
  day: {
  ...
  night: {
    page: "radial-gradient(circle at top left, #120a2d 0%, #0a1b2b 45%, #060918 100%)",
...
export function DashboardUI({
  ...
  const [theme, setTheme] =
    useState<ThemeMode>(inferThemeByTime(new Date()));
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setTheme(inferThemeByTime(new Date()));
    }, 60_000);
...
return (
  ...
  >
    ...
    <WorkflowBuilderPanel
      workflowName={workflowBuilder.name}
      nodes={workflowBuilder.nodes}
    />

    <SecurityGuardianPanel
      warnings={securityGuardian.warnings}
    />
