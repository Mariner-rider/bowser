...
type DashboardSnapshot = {
  ...
  marketplace?: {
    agents: Array<{ name: string; version: string; description: string; installed: boolean; publisher?: string; priceUsd?: number }>;
  };
  knowledgeBrain?: {
    topic: string;
    summary: string;
...
export function DashboardUI({ initialSnapshot, onPauseAgent, onStopAgent }: DashboardUIProps) {
  ...
  const marketplace =
    ...
    ({
      agents: [
        { name: "SEO Agent", version: "1.0.0", description: "Optimize website content", installed: false, publisher: "Rivinity Labs", priceUsd: 0 },
        { name: "Stock Market Agent", version: "1.2.1", description: "Analyze market trends", installed: false, publisher: "Quant Guild", priceUsd: 19 },
        { name: "Coding Assistant", version: "2.0.0", description: "Generate and review code", installed: true, publisher: "Rivinity Labs", priceUsd: 49 },
      ],
    } as const);
  ...
  const handlePublishAgent = (submission: {
    name: string;
    version: string;
    description: string;
    capabilities: string[];
    ...
    setSnapshot((prev) => ({
      ...
      marketplace: {
        agents: [
          ...(prev.marketplace?.agents ?? []),
          {
            name: submission.name,
            version: submission.version,
            description: submission.description,
            installed: false,
            publisher: submission.publisherName,
  ...
  return (
    ...
    >
      ...
      <MarketplacePanel agents={marketplace.agents} theme={theme} onPublishAgent={handlePublishAgent} />
      <KnowledgeBrainPanel
